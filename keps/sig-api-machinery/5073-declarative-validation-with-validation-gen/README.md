...
The following lint rules enforce the lifecycle contract for `+k8s:alpha` / `+k8s:beta` wrappers. All enforcement is at generation/lint time — the runtime does not read `since:` values and rules never change state on binary-version bump.

**Rules in `validation-gen`** (`lint_rules.go`, run via `hack/update-codegen.sh`):

*   **`alphaBetaPrefix`**: `+k8s:alpha` and `+k8s:beta` must be the outermost wrapper; nesting is rejected. Payload must be a validator tag.
*   **`validationStability`**: Inner validator tag stability must be ≥ the context level inferred from the package path (`*/v1alphaN` → Alpha, `*/v1betaN` → Beta, otherwise Stable).
*   **`lifecycleSinceRequired`** (v1.37): In `k8s.io/api/**` packages, `+k8s:alpha` / `+k8s:beta` without a `since:` argument are rejected.
*   **`lifecycleStaleSince`** (v1.37): Compares `since:` values against the current release (via `--current-release=1.N` or auto-read from `DefaultKubeBinaryVersion`). Warnings print without failing generation; errors fail:
    *   `+k8s:alpha`: warning at N+1, error at N+2+.
    *   `+k8s:beta`: warning at N+2, error at N+3+.
    *   Per-field exemptions via `// +k8s:validation-lifecycle-exempt=<reason>`.

**Rule in `kube-api-linter`** (`hack/kube-api-linter/kube-api-linter.yaml`, run via `make lint`):

*   **`conflictingmarkers`** (`declarative_validation_lifecycle_alpha_vs_beta`, v1.36): A field may not carry both `+k8s:alpha(...)` and `+k8s:beta(...)` wrappers.
